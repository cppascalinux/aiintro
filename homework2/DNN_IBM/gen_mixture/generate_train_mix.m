function generate_train_mix( noise_ch, dB, repeat_time, train_list, TMP_STORE, write_wav)
    warning('off','all');
    global min_cut;
    ...
    while ischar(tline)
        ...
        for var_ind = 1:repeat_time
            ...
            save_wav_mix_path = [TMP_STORE filesep 'db' num2str(dB) filesep 'mix' filesep 'WAV_train' filesep];
            if ~exist(save_wav_mix_path,'dir'); mkdir(save_wav_mix_path); end;
            sig = mix;
            sig = sig/max(abs(sig))*0.9999;
            audiowrite([save_wav_mix_path tline], sig,16e3);
            ...
            save_wav_clean_path = [TMP_STORE filesep 'db' num2str(dB) filesep 'clean' filesep 'WAV_train' filesep];
            if ~exist(save_wav_clean_path,'dir'); mkdir(save_wav_clean_path); end;
            sig = s;
            sig = sig/max(abs(sig))*0.9999;
            audiowrite([save_wav_clean_path tline], sig,16e3);
