function generate_test_mix( noise_ch, dB, test_list, TMP_STORE, write_wav)
    warning('off','all');
    global min_cut;
    ...
    while ischar(tline)
        ...
        save_wav_mix_path = [TMP_STORE filesep 'db' num2str(dB) filesep 'mix' filesep 'WAV_test' filesep];
        if ~exist(save_wav_mix_path,'dir'); mkdir(save_wav_mix_path); end;
        sig_mix = mix;
        sig_mix = sig_mix/max(abs(sig_mix))*0.9999;
        audiowrite([save_wav_mix_path tline], sig_mix,16e3);
        
        save_wav_clean_path = [TMP_STORE filesep 'db' num2str(dB) filesep 'clean' filesep];
        if ~exist(save_wav_clean_path,'dir'); mkdir(save_wav_clean_path); end;
        save_wav_clean_path = [TMP_STORE filesep 'db' num2str(dB) filesep 'clean' filesep 'WAV_test' filesep];
        if ~exist(save_wav_clean_path,'dir'); mkdir(save_wav_clean_path); end;
        sig_clean = s;
        sig_clean = sig_clean/max(abs(sig_clean))*0.9999;
        audiowrite([save_wav_clean_path tline], sig_clean,16e3);
